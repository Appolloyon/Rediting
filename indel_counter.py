...
# This program written by Christen M. Klinger, Dept. of Cell Biology,
# University of Alberta, Edmonton, Alberta, Canada
# email: cklinger@ualberta.ca
# Permission is granted to modify, use, and distribute this program freely
# If you find this program useful, please consider citing the relevant
# publication or acknowleding the author in your work

import re
import argparse

parser = argparse.ArgumentParser(
    description = "Calculates position of indels against a reference sequence",
    epilog = """This program assumes that the reference and query sequence for
    a given gene are provided in the same file in aligned FASTA format. The aligned
    file can be generated by any number of programs, e.g. MAFFT, MUSCLE, etc.
    It also assumes that the header line for the references sequence will have the
    string 'reference' somewhere in it while the test sequence does not contain this
    string in its header. Output is given as two csv files per gene""")
...
for infile in args.infiles:
    name = infile.split('.')[0] # note this is the name used for outfiles
    ...
    ref_counter = 1 # keep track of position in reference sequence
    ...
    # consider only the region present in both sequences
    ...
    with open(out_insert,'w') as o1, open(out_deletion,'w') as o2:
        ...
        o1.write("Identity,Position,Number of Inserts" + '\n' * 2)
        o2.write("Identity,Position,Number of Deletions" + '\n' * 2)
